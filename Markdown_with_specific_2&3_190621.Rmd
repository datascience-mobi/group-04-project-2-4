...
Draw a barplot which have the hichst foldchange after elotinib treatment
```{r}
...
###generation of a colour vector according to the cancer type. 
```{r}
...
Draw a scatterplot which include the GI50 values agains the EGR1 expression revative to the untreated controlle. ERG1 is atrainscripional factor and ist assocciated with the activiation of tumor suppressor genes like p53/TP53 and TGFB1, and play an importent role in the regulation of growth factor responses.
```{r}
#Scatter plot
...
```
 Only 25 % can be discribed by EGR1- expression, whicht makes a linear regression model kind of unfitted. 


### Expressionsdaten von EGFR Expressione relativ to controle --> Does erlotinib treatment affect her1/ EGFR expression?
```{r}
