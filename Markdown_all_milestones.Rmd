...
cellline_untreated <- sapply(colnames(e_untreated), function(x){
   colnames(annotation_sorted_by_cell_lines)[which(x == annotation_sorted_by_cell_lines, arr.ind = TRUE)[2]]
 })
colnames(e_untreated) <- cellline_untreated
...
```{r figEGR1 , fig.width = 6, fig.hight =6, fig.asp = 1.1}
#Coloring according to cancertype
...
plot(LogGI50_59_celllines, e_foldchange_normalized ["EGR1",], 
     col = e_color_cancertype, 
     pch = 19, 
     xlab = "logGI50", 
     ylab = "EGR1 Expression (log2, relative to control)",        
     main = "Erlotinib 24h", col.main = "deepskyblue")
...
text(LogGI50_59_celllines[labeled_celllines], e_foldchange_normalized ["EGR1", labeled_celllines], 
...
```{r comment = NA}
linearMod_EGR1 <- lm(LogGI50_59_celllines ~ e_foldchange_normalized ["EGR1",])  # build linear regression model on full data
...
heatmap_data <- sapply(1:length(pathways), function(x){ #for each pathway
  sapply(1:ncol(e_foldchange_normalized), function(y){ #for each sample
    ...
    mean(e_foldchange_normalized[pathways[[x]], y]) 
  })
})
