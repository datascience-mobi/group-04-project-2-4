...
mean_FC_abs <- as.data.frame(cbind(cellline= names(mean_FC_abs[1:20]), abs_foldchange = mean_FC_abs[1:20]))
'ggplot(data=mean_FC_abs, aes(x=cellline, y=abs_foldchange)) +
    geom_bar(stat="identity")+
    ggtitle("Genes with highest mean in absolute FC")+
    scale_x_discrete(limits= rev(rownames(mean_FC_abs)))+
    coord_flip()
...
FC_samples_with_highest_mean_FC <- data.matrix(as.data.frame(sapply(mean_FC_abs$cellline, function(x){
...
In our third milestone, we want to identify those genes, which are most regulated by erlotinib.Therefore, we perform a MA plot to illustrate and select the genes, with the highest fold change. Thereafter, we further reduce the genes obtained through the MA plot according to their significance using a volcano plot. Drawing a Venn diagram, we want to illustrate, how many genes are left out due to their lower significance. With those biomarkers, we draw a density to plot of up- and downregulated genes to compare the gene expression before and after treatment. 
...
#create a data frame with the variables M and A of a MA-plot as well as one column, which shows, if the absolute fold change is greater than 0.25 
...
#create a MA plot, where genes, with FC_abs > 0.25 are colored and those genes with FC_abs > 0.5 are labeled
ggplot(data = MA)+ 
...
#save the highlighted biomarkers, obtained through the MA plot in biomarkers_MA
biomarkers_MA <- rownames(MA)[which(abs(MA$M) > 0.25)]
```

### Select biomarker with high foldchange and high significance
Create a **volcano plot** to find the genes, which have in addition to a high fold change, also a very high significance
...
#calculate the mean of foldchange over all cell lines for each gene
e_foldchange_mean_over_cell_lines <- rowMeans(e_foldchange) 

#determine the p-value for a paired two-sample t-test between the gene expression of treated and untreated samples
...
FDR_values <- p.adjust(p_values, method = "BH", n = length(p_values))#calculate FDR with benjamini-hochberg (BH), because most of the genes are not regulated at all and with a p-value < 0.05 we would have more false-positives

#create a matrix with one column for the mean foldchange and one column for FDR_values for every gene
volcano_plot_mat <- cbind(e_foldchange_mean_over_cell_lines, FDR_values)

#draw a volcano plot, where those genes, which have a FC_abs > 0.25, but also an FDR_value > 10e^-9 are labeled and highlighted in red 
...
#save the highlighted biomarkers, obtained through the volcano plot in biomarkers_volcano
biomarkers_volcano <- rownames(volcano_plot_mat)[which(abs(volcano_plot_mat[, 1]) >  0.25  & volcano_plot_mat[, 2] < 10e-9)]
```


### Compare biomarkers of high foldchange and high significance
A **Venn Diagram** is drawn to compare the most regulated genes obtained through the MA plot and through the volcano plot. This shows, how many genes were sorted out due to a lower FDR_value.
```{r warning = FALSE, message = FALSE}
#load library for Venn Diagram
library(VennDiagram)

#draw a Venn Diagram with biomarkers of MA and Volcano plot
par(oma = c(2, 2, 2, 2))
venn.plot <- venn.diagram(
  x = list(
...
A **density plot** is drawn only with biomarkers identified by volcano plot to show, how the gene expression before and after erlotinib treatment changed. Therefore, we have to separate between up- and downregulated to make sure that positive and negative fold changes do not cancel each other out.
```{r}
#create a new vector, where up- and only downregulated volcano biomarkers are separated
...
```{r figEGR1 , fig.show = "hold", fig.align = "default", warning = FALSE, fig.width=4.5}
library(cowplot)
...
pic1 <- ggdraw() + draw_image(url, scale = 2.6) 
...
text(LogGI50_59_celllines[labeled_celllines], e_foldchange ["EGR1", labeled_celllines], 
     ...
     cex = 0.7,
     pos = 4) #position of text at the right of the point

mtext(side = 3, line = -1, text = "r= 0.6792 (***)", adj = c(0.01))}


# tranform plot into png 
png("erlotinib_GI50_24h.png", width = 400, height = 800)
...
plot_grid(pic1, pic2, align = "h", nrow = 1)
