...
pheatmap(t(progeny_heatmap),
         ...
         col = rev(brewer.pal(10, "RdBu")))
```
When camparing the two heatmaps you can see that a different number of clusters was generated by the elbow plot. While the heatmap according to the supplementary material S2 we have 3 Clusters, the second heatmap have only 2 clusters. In the second heatmap the different clusters are clealy visible, while the first heatmap does not show a very clear clusters exepte from the cluster on the left side. 
