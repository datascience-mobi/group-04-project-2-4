...
legend("topright", 
       ...
       inset = c(-0.18, 0))
#label points
labeled_celllines_PCA <- names(pca$rotation[ , 1])[pca$rotation[ , 1] > 0.14]
text(pca$rotation[ ,1][labeled_celllines_PCA], 
     pca$rotation[ ,2][labeled_celllines_PCA], 
     labeled_celllines_PCA,
     pos = 2,
     cex = 0.6)
```
...
biomarker_final <- sapply (names(cor_biomarkers), function (x) {
  names(cor_biomarkers) [which x == names(cor_top_500_genes)]})
```
...
foldchange_biomarkers <- sapply(biomarkers_volcano, function(x){
  e_foldchange[x, ]
})
...
e_treated_biomarkers_volcano <- sapply(biomarkers_volcano, function(x){
  e_treated[x, ]
})
e_untreated_biomarkers_volcano <- sapply(biomarkers_volcano, function(x){
  e_untreated[x, ]
}) 
