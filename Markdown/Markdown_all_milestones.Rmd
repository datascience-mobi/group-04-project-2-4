...
Now we want to identify the celllines with the highest variance. That means that the foldchange is far apart from the mean value. Therfore wie calculate the variance of the foldchange and save the results under the variable "var_cellline". After that we create a verctor called "cellline_top15" which contains the names of the top 15 celllines with the highest variance. After thet we used the matrix "annotation" which contains the full sample name, the drug, the cellline and the cancertyp, to generate a vector called "cancertypes_top15". This vector has the cacncertype of our top 15 celllines in it. Then we use the cbind-function to generate a matrix which contains the Rank, the cellline, the variance value and the cancertype of our top 15 celllines. This matrix is henceforth referred as "table_celllines_var_top15".
...
First we have to list all the genes from table S2 of the supplementary material and save them into different vectors. After that we create a list of all pathways, therfore we used the function list to merge all the different vectors. The list is now called "pathways".
