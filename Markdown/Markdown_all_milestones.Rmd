...
To begin, the erlotinib target EGFR is downregulated by the drug. EGFR, also called epidermal growth factor receptor, is stimulated, as its name implies by epidermal growth factors. After its activation many different pathways are induced like cell proliferation, survival or growth. Erlotinib also binds to the EGFR and therefore blocks those signalling pathway inhibiting tumor progression.   
...
Firstly, we draw an **MA plot** to compare the fold change to the mean expression of all genes. Thereby, we select only those genes, which have a mean fold change higher than 0.25 or a mean fold change lower than -0.25.
```{r message = FALSE, warning=FALSE}
