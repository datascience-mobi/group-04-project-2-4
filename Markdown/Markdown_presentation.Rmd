...
```{r figCHAC1 , fig.show = "hold",  fig.width = 7, fig.height = 7, warning = FALSE,  message = FALSE, echo = FALSE}

# draw scatter plot of fold change of CHAC1 against LogGI50 and color them according to the cancer type
par(mar = c(4, 4, 4, 10), xpd = "TRUE")
plot(LogGI50_59_celllines, e_foldchange ["CHAC1",], 
     col = e_color_cancertype, 
     pch = 19, 
     xlab = "logGI50", 
     ylab = "Fold change",        
     main = "CHAC1 expression", 
     cex.main = 1.2)
...
# add linear regression line and set clipping limit
clip(-7.13, -3.88, -0.15, 1.9)  
abline(lm (e_foldchange["CHAC1", ]~ LogGI50_59_celllines))

# perform Pearson correlation, to see how well LogGI50 and the fold change correlate with each other 
cor_CHAC1 <- cor.test(LogGI50_59_celllines, e_foldchange["CHAC1",], method = "pearson")
cor_CHAC1_estimate <- round(cor_CHAC1$estimate, digits = 3)

# add title to scatter plot, which includes the correlation value
cor_text <- paste("r = ", cor_CHAC1_estimate)
mtext(side = 3, line = -1.2, text = cor_text, adj = c(0.01))
```
