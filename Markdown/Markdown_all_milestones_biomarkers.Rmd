...
Venn Diagramm of celllines from heatmap cluster 2 and top15 variance celllines
```{r message = FALSE}
library(VennDiagram)
...
venn.plot <- venn.diagram(
  x = list(
    "Cellines Heatmap Cluster 2" = heatmap_genes_cluster2,
    "top15 variance celllines" = table_cell_lines_var_top15[, 2] #column 2 contains celllines
    ),
  filename = NULL, 
  fill = c("blue", "red"), 
...
MA_labeled <- MA[which(abs(MA[ , "M"]) > 0.25) , ]
...
biomarkers_final <- unlist(sapply(names(cor_biomarkers), function(x){
  x[which (x == names(cor_top_500_genes))]
}))

biomarkers_final <- gsub(pattern = ".cor", replacement = "", x = names(biomarkers_final))
