...
#test coloring for volcano plot
volcano_results <- rbind(abs(e_foldchange_mean_over_cell_lines), FDR_values)

color_palette_volcano <- c("purple", "firebrick1", "deepskyblue")
names(color_palette_volcano) <- c("FDR values < 0.05", "mean FC > 1", "both")

color_vector_volcano <- sapply(colnames(volcano_results), function(x) { #for each gene
  count <- 0
  if (volcano_results[2, x] < 0.05){
    unname(color_palette_volcano[1])
    count <- 1
  } 
  if (volcano_results[1, x] > 1){
    unname(color_palette_volcano[2])
    count <- 1
  } 
  if (volcano_results[2, x] < 0.05 & volcano_results[1, x] > 1){
    unname(color_palette_volcano[3])
    count <- 1
  }
  if (count == 0){
    "black"
...
plot(e_foldchange_mean_over_cell_lines, -log10(FDR_values), pch=20, main="Volcano plot", xlim=c(-1,2), col= unname(color_vector_volcano))
